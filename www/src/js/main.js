...
function setupEventListeners() {
    ...
    // ===== Touch handling: 1 finger = draw, 2 fingers = pan/scroll =====
    const canvasArea = document.querySelector('.canvas-area');
    let touchMode = null; // 'draw' | 'pan' | null
    let panLastTouch = null; // { x, y } for tracking pan delta
    let touchWasTwoFinger = false; // Track if gesture involved 2 fingers
    
    let touchStartHandler = (e) => {
        if (e.touches.length === 2) {
            // Two-finger gesture → pan mode
            e.preventDefault();
            touchMode = 'pan';
            touchWasTwoFinger = true;
            // Cancel any ongoing drawing
            if (state.isDrawing) {
                const mouseUp = new MouseEvent('mouseup', { bubbles: true, cancelable: true });
                canvas.dispatchEvent(mouseUp);
            }
            // Calculate midpoint of two fingers for panning
        ...
        } else if (e.touches.length === 1 && !touchWasTwoFinger) {
    ...
    let touchMoveHandler = (e) => {
        if (touchMode === 'pan' && e.touches.length >= 2) {
            ...
            if (panLastTouch && canvasArea) {
                const dx = panLastTouch.x - midX;
                const dy = panLastTouch.y - midY;
                canvasArea.scrollLeft += dx;
                canvasArea.scrollTop += dy;
            }
            
            panLastTouch = { x: midX, y: midY };
        } else if (touchMode === 'draw' && e.touches.length === 1 && state.isDrawing) {
            e.preventDefault();
            const touch = e.touches[0];
    ...
    let touchEndHandler = (e) => {
        if (e.touches.length === 0) {
            // All fingers lifted
            if (touchMode === 'draw' && !touchWasTwoFinger) {
            ...
            // Went from 2 fingers to 1 — stay in pan mode, don't start drawing
            touchWasTwoFinger = true;
        }
    };
    ...
    // Also prevent default on canvas-area for two-finger pan gestures
    if (canvasArea) {
        canvasArea.addEventListener('touchmove', (e) => {
            if (e.target === canvas || canvas.contains(e.target)) {
                if (state.isDrawing || touchMode === 'pan') {
                    e.preventDefault();
                }
            }
        }, { passive: false });
